...
CREATE OR REPLACE FUNCTION get_products_by_brand(brand_id INTEGER) RETURNS TABLE (
        product_id INTEGER,
...
CREATE OR REPLACE FUNCTION get_brands_with_products_count() RETURNS TABLE (
        brand_id integer,
...
CREATE OR REPLACE FUNCTION get_products_by_category_and_section(category_id integer, section_id integer) RETURNS TABLE (
        product_id INT,
        product_name VARCHAR(45),
        product_price MONEY,
        product_description VARCHAR(200),
...
CREATE OR REPLACE FUNCTION get_completed_orders_by_product(product_id INT) RETURNS TABLE (
        order_id INT,
        user_id INT,
...
WHERE oh.status_id = 3
    AND op.product_id = get_completed_orders_by_product.product_id
